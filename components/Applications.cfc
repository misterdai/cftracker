component displayName = 'Applications' hint = 'Provides access to Application instances that currently exist within this CFML instance.' output = false {

	public Applications function init(
	)	description = 'Initialises the Applications component.'
		...
		output = false
	{
		...
		variables.methods.initialised = local.class.getMethod('isInited', variables.mirror);
		if (ListFirst(variables.version) Gte 10) {
			variables.methods.settings = local.class.getMethod('getApplicationSettingsMap', variables.mirror);
		} else {
